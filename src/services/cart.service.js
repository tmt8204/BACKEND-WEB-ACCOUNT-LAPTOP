...
class CartService {
    ...
    async addItemToCart(userId, data) {
        try {
            ...
            if (!item_id || !product_type) {
                throw new Error('item_id and product_type are required');
            }
            ...
            // Get the parent Product document to check is_active
            ...
            // Digital products: quantity must always be 1 (each item is unique)
            ...
            // Check if this item_id already exists in cart
            let cart = await cartRepository.findCartByUserId(userId);
            if (!cart) {
                cart = await cartRepository.createCart(userId);
            }
    ...
    async updateItemQuantity(userId, cartItemId, quantity) {
        try {
            ...
            // Digital products cannot change quantity
